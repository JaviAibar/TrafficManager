...
public class PlayGeneralTesting : MonoBehaviour
{
    ...
    public IEnumerator TrafficLightsTiming()
    {
        ...
        Assert.AreEqual("Red", trafficLight.spriteRenderer.sprite.name);

        Assert.AreNotEqual("Green", trafficLight.spriteRenderer.sprite.name);
        Assert.AreNotEqual("Yellow", trafficLight.spriteRenderer.sprite.name);

        yield return new WaitForSeconds(1);
        Assert.AreEqual("Green", trafficLight.spriteRenderer.sprite.name);

        Assert.AreNotEqual("Red", trafficLight.spriteRenderer.sprite.name);
        Assert.AreNotEqual("Yellow", trafficLight.spriteRenderer.sprite.name);

        yield return new WaitForSeconds(3);
        Assert.AreEqual("Yellow", trafficLight.spriteRenderer.sprite.name);
